    ...
    <div style={{ minHeight: '100vh', background: '#0a0a0a', color: '#f5f3ee', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '2rem' }}>
      <div style={{ width: '100%', maxWidth: 480, border: '1px solid rgba(255,179,0,0.2)', borderRadius: 24, padding: '2rem', background: 'rgba(17,18,20,0.95)', boxShadow: '0 20px 60px rgba(0,0,0,0.35)' }}>
        ...
        <form onSubmit={handleSubmit} style={{ display: 'grid', gap: '0.9rem' }}>
          <label style={{ display: 'grid', gap: '0.35rem', color: '#f5f3ee' }}>
            <span>Email</span>
            <input
              type="email"
              value={email}
              onChange={(event) => setEmail(event.target.value)}
              style={{ padding: '0.9rem 1rem', borderRadius: 12, border: '1px solid rgba(255,179,0,0.2)', background: '#111214', color: '#f5f3ee' }}
          ...
          {error ? (
            <div style={{ background: 'rgba(220,38,38,0.12)', color: '#fca5a5', border: '1px solid rgba(220,38,38,0.35)', borderRadius: 12, padding: '0.75rem 1rem' }}>
              {error}
            </div>
          ) : null}

          <button type="submit" disabled={isLoading} className="btn btn-primary" style={{ justifyContent: 'center' }}>
